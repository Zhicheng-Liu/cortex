...
HashTableEc * hash_table_ec_new(int number_bits, int bucket_size, int max_rehash_tries, short kmer_size){ 
  ...
  HashTableEc *hash_table = malloc(sizeof(HashTableEc));
  ...
  hash_table->table = calloc(hash_table->number_buckets * hash_table->bucket_size, sizeof(ElementEc));
...
boolean hash_table_ec_find_in_bucket(Key key, long long * current_pos, boolean * overflow, HashTableEc * hash_table, int rehash){
  ...
  *current_pos   = (long long) hashval * hash_table->bucket_size;   //position in hash table
  ...
  while( (i<hash_table->bucket_size) &&   // still within the bucket
         (!db_node_ec_check_for_flag_ALL_OFF(&hash_table->table[*current_pos]) )  && // not yet reached an empty space
	 (!found)
	 )
    {
    ...
    if (element_ec_is_key(key,hash_table->table[*current_pos]))
      {
	found = true;
      }
    else
...
boolean hash_table_ec_apply_or_insert(Key key, void (*f)(ElementEc *), HashTableEc * hash_table){
  if (hash_table == NULL) {
    die("NULL table!");
  }
  ...
  do
    {
      found = hash_table_ec_find_in_bucket(key,&current_pos,&overflow, hash_table,rehash);
...
void hash_table_ec_traverse(void (*f)(ElementEc *),HashTableEc * hash_table){
  long long i;
 
  for(i=0;i<hash_table->number_buckets * hash_table->bucket_size;i++){
    
    if (!db_node_ec_check_for_flag_ALL_OFF(&hash_table->table[i])){
...
void hash_table_ec_traverse_passing_int(void (*f)(ElementEc *, int*),HashTableEc * hash_table, int* num){
  long long i;
  for(i=0;i<hash_table->number_buckets * hash_table->bucket_size;i++){
    if (!db_node_ec_check_status(&hash_table->table[i],unassigned)){
      f(&hash_table->table[i], num);
    }
  }
}
void hash_table_ec_traverse_passing_ints_and_path(void (*f)(ElementEc *, int*, int*, dBNodeEc**, Orientation*, Nucleotide*, char*, int),
...
ElementEc * hash_table_ec_find(Key key, HashTableEc * hash_table)
{
  if (hash_table == NULL) 
    {
      die("hash_table_ec_find has been called with a NULL table! Exiting");
    }
...
void hash_table_ec_print_stats(HashTableEc * hash_table)
{
  int k;
  printf("Collisions:\n");
  for(k=0;k<10;k++)
...
long long hash_table_ec_get_unique_kmers(HashTableEc * hash_table)
{
